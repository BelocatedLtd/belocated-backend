...
export const startRefChallenge = async () => {
	...
	const hasOngoingChallenge = pastChallenges.some(
		(pc) => pc.status === 'Ongoing',
	)

	if (hasOngoingChallenge) {
		console.error(
			"There's still an ongoing challenge, so new challenge cannot be created",
		)
		return
		// Send email to admin
	}
...
export const endRefChallenge = async () => {
	...
	contestants.forEach((item: any) => {
		counts[item] = (counts[item] || 0) + 1
	})

	// Sort the contestants based on counts
	const sortedContestants = Object.keys(counts).sort(
	...
	const firstPositionWallet = await Wallet.findOne({
	...
	const secondPositionWallet = await Wallet.findOne({
	...
	// Rewarding first position
	const thirdPositionWallet = await Wallet.findOne({
	...
	try {
		...
		await User.updateMany(
			{},
			{ $set: { referralChallengePts: 0, referralChallengeReferredUsers: [] } },
		)
