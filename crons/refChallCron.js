...
export const startRefChallenge = async() => {
...
// Create new challenge and set countdown timer
    ...
    const newChallenge = await RefChallenge.create({
        firstId: "",
        secondId: "",
        thirdId: "",
        totalRefUsers: 0,
...
export const endRefChallenge = async() => {
    ...
    completedRefChallenge.firstId = contestantFirstPosition;
    completedRefChallenge.secondId = contestantSecondPosition;
    completedRefChallenge.thirdId = contestantThirdPosition;
    ...
    const firstPositionWallet = await Wallet.findOne({userId: "contestantFirstPosition"});
    if (firstPositionWallet) {
     ...
     const secondPositionWallet = await Wallet.findOne({userId: "contestantSecondPosition"});
    if (secondPositionWallet) {
     ...
     // Rewarding first position
     const thirdPositionWallet = await Wallet.findOne({userId: "contestantThirdPosition"});
    ...
    try {
        ...
        await User.updateMany({}, { $set: { referralChallengePts: 0, referralChallengeReferredUsers: [] } });
    ...
    const emailSent = await sendEmail(subject, message, send_to, username)
